...
static int LevelUpTbl[] = { // 經驗值 等級 換算表
0,			//  0 EX=(lv^4)/10
0,			//  1
	        2,        8,       25,       62,      129,
...
{	
	#ifdef _NEWOPEN_MAXEXP
	return min(arraysizeof( NeedLevelUpTbls )-1,getjz_maxlevel());
	#else
...
{
#ifdef _NEWOPEN_MAXEXP
	if( level >= (min(arraysizeof( NeedLevelUpTbls ),getjz_maxlevel())) ){
		return -1;
	}
...
int CHAR_LevelUpCheck( int charaindex , int toindex)
{
	...
	while( 1 ){
		...
		//這裡應該做等級限制的判斷
		if((level>139)&&(CHAR_getInt(charaindex,CHAR_TRANSMIGRATION)<gettransmaxlevel()))return iRet;
...
#endif
#ifdef _TEACHER_SYSTEM
					iGetFame = feedpoint/20; // 導師獲得學生所得聲望的 5% (1/20)
					// 檢查所得的聲望有沒有大於0
					if(iGetFame > 0){
						// 檢查有沒有導師
...
int PETFUSION_FusionPetSub( int charaindex, int Subindex1, int Subindex2, int *work, int *skill)
{
	...
	if( PET_getBaseAndSkill( charaindex, Subindex1, base, petskill, 0) == FALSE )
		return 0;
	if( CHAR_getInt( Subindex1, CHAR_LV) < 80 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}
	...
	if( PET_getBaseAndSkill( charaindex, Subindex2, base, NULL, 0) == FALSE )
		return 0;
	if( CHAR_getInt( Subindex2, CHAR_LV) < 80 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}
...
BOOL PETFUSION_FusionPetMain( int charaindex, int Mainindex, int *work, int *skill)
{
	...
	if( PET_getBaseAndSkill( charaindex, Mainindex, base, petskill, 0) == FALSE )
		return -1;
	if( CHAR_getInt( Mainindex, CHAR_LV) < 80 ){//等級銷弱
		for( i=0; i<4; i++)	{
			base[i] = base[i]*0.8;
		}
	}
...
#endif
{
	...
	if( LV > 130 ) LV = 130;
	total = ( (float)total1/100 ); // 瑪蕾最高 total1=200
...
void CHAR_CheckWaterStatus( int charaindex)
{
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORKMAPFLOORTYPE) == 1 &&
		...
		if( defhp <= 0 ) defhp = 1;
		myhp = myhp - defhp;
		sprintf( token, "因無法呼吸，扣%d HP。", defhp);
...
int CHAR_init_exp(char *path)
{
	...
	while(1){		
		...
		NeedLevelUpTbls[listindex+1]=atoi(token);
	}
	fclose(fp);
	return TRUE;
}