...
void CHAR_CHAT_DEBUG_hp( int charindex , char *message )
{
  ...
  easyGetTokenFromString( message , 2 , token, sizeof(token));
  if( strlen(token)==0){
  	CHAR_talkToCli( charindex, -1,"忽略參數->帳號", CHAR_COLORWHITE);
  }else{
...
void CHAR_CHAT_DEBUG_info( int charindex , char *message )
{
	...
	if( !CHAR_CHECKINDEX( index)) {
		CHAR_talkToCli( charindex, -1 , "該index無法使用。" , CHAR_COLORWHITE );
		return;
	}
	...
	if( CHAR_getInt( index, CHAR_WHICHTYPE ) == CHAR_TYPEPET ){
		snprintf( line, sizeof(line),
			  "體:%d 力:%d 丈:%d 早:%d 耐:%d 攻:%d 守:%d 敏:%d 忠修:%d 忠變:%.2f 編碼:%s",
		...
		snprintf( line, sizeof(line),
			  "體:%d 力:%d 丈:%d 早:%d 耐:%d 攻:%d 守:%d 魅:%d 運:%d",
			  CHAR_getInt( index, CHAR_VITAL )/100,
			  CHAR_getInt( index, CHAR_STR )/100,
			  CHAR_getInt( index, CHAR_TOUGH )/100,
			  CHAR_getInt( index, CHAR_DEX )/100,
	...
	snprintf( line, sizeof( line),
			  "Status 毒:%d 麻:%d 默:%d 石:%d 暗:%d 亂:%d" ,
...
void CHAR_CHAT_DEBUG_sysinfo( int charindex , char *message )
{
	...
	snprintf( msg , sizeof(msg),
			  "服務端評估數據:sys:%2.4f net:%2.4f battle:%2.4f char:%2.4f",
...
void CHAR_CHAT_DEBUG_level( int charaindex, char* message )
{
  ...
  if( strlen(buf1)==0){
  	CHAR_talkToCli( charaindex, -1,"忽略參數->帳號", CHAR_COLORWHITE);
  }else{
		for(i=0;i<getFdnum();i++){
			i_use=CONNECT_getUse(i);
			if(i_use){
	...
	sprintf( buf1, "LEVEL 設為%d。", leveltemp);
...
void CHAR_CHAT_DEBUG_delitem( int charindex , char *message )
{
	...
	if( strcmp( message, "all") == 0 ) {
		...
		if( equipplace < CHAR_STARTITEMARRAY || equipplace > CHAR_MAXITEMHAVE -1 ) {
			CHAR_talkToCli( charindex, -1, "號碼不正確。",  CHAR_COLORWHITE);
			return;
		}
...
void CHAR_CHAT_DEBUG_addsk( int charaindex, char* message )
{
	...
	if( level > 100 ){
		CHAR_talkToCli( charaindex, -1, "等級不正確",  CHAR_COLORWHITE);
		return;
	}
	if( level < 1 ) level = 1;
...
void CHAR_CHAT_Kusabana( int charaindex, char* message ){
	...
	if( MAP_walkAbleFromPoint( ff, fx, fy, FALSE ) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "無法走了。",  CHAR_COLORWHITE);
		return;
	}
...
void CHAR_CHAT_Fukuwa( int charaindex, char* message ){
    ...
    if( message[0] == '[' && message[stringlen-1] == ']' ){
		CHAR_talkToCli( charaindex, -1, "語言沒有轉換。",  CHAR_COLORWHITE);
		return ;
    }
...
void CHAR_CHAT_DEBUG_petmake( int charaindex, char* message )
{
  ...
  if( strlen(token)==0){
...
#ifdef _JZ_IMPROVE_GMCOMMAND
	snprintf( msgbuf,sizeof( msgbuf), "寵物製作成功\!index[%d]",ret);
	CHAR_talkToCli( fd_charindex, -1, msgbuf,  CHAR_COLORWHITE);
#endif
	CHAR_talkToCli( charaindex, -1, "你獲得了一隻寵物",  CHAR_COLORWHITE);
	...
	if( level != -1 ){
		...
		if( level < 0 ) level = 0;
		level = level - CHAR_getInt( ret, CHAR_LV )+1;
		if( level > 0 ){
			int k;
...
void CHAR_CHAT_DEBUG_deletepet( int charaindex, char* message )
{
	...
	CHAR_talkToCli( charaindex, -1, "清除身上所有寵物。",  CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_reset( int charaindex, char* message )
{
	...
	if( i >= MAXMESS_LISTNUM ){
		CHAR_talkToCli( charaindex, -1, "指令錯誤",  CHAR_COLORRED);//CHAR_COLORYELLOW
		return;
	}
	switch( i){
	case ALLRESET:
	case 0:
		if( !ENEMYTEMP_reinitEnemy() || !ENEMY_reinitEnemy() ){
			CHAR_talkToCli( charaindex, -1, "重新讀取敵人基本資料失敗。",  CHAR_COLORRED);
			return;
		}
		if( i != ALLRESET ) break;
	case 1:
		if( !GROUP_reinitGroup() || !ENCOUNT_reinitEncount() ){
			CHAR_talkToCli( charaindex, -1, "重新讀取遭遇團隊資料失敗。",  CHAR_COLORRED);
		}
		if( i != ALLRESET ) break;
	case 2:
		if( !MAGIC_reinitMagic() || !ATTMAGIC_reinitMagic() ) {
			CHAR_talkToCli( charaindex, -1, "重新讀取咒術資料失敗。",  CHAR_COLORRED);
		}
		if( i != ALLRESET ) break;
	case 3:
		MAPPOINT_resetMapWarpPoint( 1);
		if( !MAPPOINT_loadMapWarpPoint() ){
			CHAR_talkToCli( charaindex, -1, "重新讀取跳躍點資料失敗。",  CHAR_COLORRED);
		}
		if( i != ALLRESET ) break;
	case 4:
		if( !PETSKILL_reinitPetskill() ) {
			CHAR_talkToCli( charaindex, -1, "重新讀取寵物技能資料失敗。",  CHAR_COLORRED);
		}
		if( i != ALLRESET ) break;
	case 5:
		LoadPetTalk();
	...
	CHAR_talkToCli( charaindex, -1, "重新讀取遊戲資料。",  CHAR_COLORYELLOW);
}

void CHAR_CHAT_DEBUG_enemyrestart( int charaindex, char* message )
{
	if( ENEMYTEMP_reinitEnemy()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取敵人基本資料。",  CHAR_COLORWHITE);
	}else {
		CHAR_talkToCli( charaindex, -1, "再次讀取敵人基本資料失敗。",  CHAR_COLORWHITE);
	}

	if( ENEMY_reinitEnemy()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取敵人資料。",  CHAR_COLORWHITE);
	}else {
		CHAR_talkToCli( charaindex, -1, "再次讀取敵人基本資料失敗。",  CHAR_COLORWHITE);
	}

	if( GROUP_reinitGroup()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取團隊資料。",  CHAR_COLORWHITE);
	}else {
		CHAR_talkToCli( charaindex, -1, "再次讀取團隊資料失敗。",  CHAR_COLORWHITE);
	}
	if( ENCOUNT_reinitEncount()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取遭遇資料。",  CHAR_COLORWHITE);
	}
	else {
		CHAR_talkToCli( charaindex, -1, "再次讀取遭遇資料失敗。",  CHAR_COLORWHITE);
	}

	if( MAGIC_reinitMagic()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取咒術資料。",  CHAR_COLORWHITE);
	}
	else {
		CHAR_talkToCli( charaindex, -1, "再次讀取咒術資料失敗。",  CHAR_COLORWHITE);
	}
...
#endif
	if( PETSKILL_reinitPetskill()) {
		CHAR_talkToCli( charaindex, -1, "再次讀取寵物技能資料。",  CHAR_COLORWHITE);
	}
	else {
		CHAR_talkToCli( charaindex, -1, "再次讀取寵物技能資料失敗。",  CHAR_COLORWHITE);
	}
	
	LoadPetTalk();	// Arminius 8.15 pet talk
	CHAR_talkToCli( charaindex, -1, "再次讀取寵物講話資料。", CHAR_COLORWHITE);
...
// shan 2001/12/18 Begin   由於改變過多，故將原先的function注掉
...
{	
	...
	if( shiftbit!=-1 && ret1==FALSE && ret2==FALSE ){
		...
		if( shiftbit==-1 || strlen(token_cdkey)==0 || strlen(token_name)==0 ){
			sprintf( token, "%s", "參數不正確。正確格式：[eventsetend 任務旗標 帳號 人物名稱]");
	...
	if(find){
		...
		if( array>=event_num ){
			sprintf( token, "錯誤！！你所設的任務旗標編號已超過  圍(0~%d)。",32*event_num-1);
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORWHITE);
			return;
		}
...
/*void CHAR_CHAT_DEBUG_eventsetnow( int charaindex, char* message )
{
	...
	sprintf( token, "設定事件中%d號旗。",eventno);
	CHAR_talkToCli( charaindex, -1,token, CHAR_COLORWHITE);
}*/
// shan 2001/12/18 End

// shan 2001/12/15 Begin   由於改變過多，故將原先的function注掉
...
void CHAR_CHAT_DEBUG_debug( int charaindex, char* message )
{
	...
	snprintf( msgbuf, sizeof( msgbuf), "已設定除錯模式為%s。",
		( flg == TRUE) ? "ON" : "OFF");
...
void CHAR_CHAT_DEBUG_arrange( int charindex , char *message )
{
	...
	CHAR_complianceParameter( charindex );
	CHAR_talkToCli( charindex, -1, "設定擋格", CHAR_COLORYELLOW);
}
#endif
...
void CHAR_CHAT_DEBUG_sequence( int charindex , char *message )
{
	...
	CHAR_complianceParameter( charindex );
	CHAR_talkToCli( charindex, -1, "設定順序", CHAR_COLORYELLOW);
}
#endif
...
void CHAR_CHAT_DEBUG_setmerge_main( int charaindex, char* message, int mode )
{
	...
	if( !CHAR_CHECKITEMINDEX( charaindex, haveitemindex) ){
		CHAR_talkToCli( charaindex, -1, "號碼很奇怪。", CHAR_COLORWHITE);
		return;
	}
	itemindex = CHAR_getItemIndex( charaindex, haveitemindex);
	if( !ITEM_CHECKINDEX( itemindex)) {
		CHAR_talkToCli( charaindex, -1, "該處是否沒有任何道具,情報亦很奇怪。", CHAR_COLORWHITE);
		return;
	}
	ITEM_setInt( itemindex, ITEM_MERGEFLG, mode);
	
	CHAR_talkToCli( charaindex, -1, "寧岳白仿弘及丑綜毛仄引仄凶〔", CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_setTrans( int charaindex, char *message)
{
	...
	int i;
	int Reward=1;
	int quest,level,teq=0,max,lv;
#ifdef _JZ_TRANSX
	float table[]={437,490,521,550,578,620,630}; //各轉最高點數(減10)
#else
	float table[]={437,490,521,550,578,620};	//各轉最高點數(減10)
#endif
	if(getStringFromIndexWithDelim( message, " ", 3, token , sizeof( token ))) //紅利
	{
		Reward = atoi(token);
	}

  if(!getStringFromIndexWithDelim( message, " ", 1, token , sizeof( token ))){
		CHAR_talkToCli( charaindex, -1,"參數不正確->轉數", CHAR_COLORWHITE);
		return;
	}
	transtemp=atoi(token);
#ifdef _JZ_TRANSX
	if(transtemp<0 || transtemp > _JZ_TRANSX){
#else
	if(transtemp<0 || transtemp > 6){
#endif
		CHAR_talkToCli( charaindex, -1,"參數範圍錯誤->轉數", CHAR_COLORWHITE);
		return;
	}
	if (Reward>=1){
		quest=20*transtemp;
	...
	if(!getStringFromIndexWithDelim( message, " ", 2, token , sizeof( token ))){
  	...
  	max=(transtemp==0)?(lv-1)*3+20+10: // 0轉447
			(lv-1)*3+table[transtemp-1]/12.0+quest/4.0+(level-transtemp*85)/4.0+10+1+10+transtemp*10;
/* 1轉以上=升級點數+繼承點術+歷史任務+歷史等級+轉前祝福+誤差1點+轉後祝福+轉生紅利 */
		if (transtemp==6) max=max-20; /* 六轉時沒有轉前祝福與轉後祝福 */
#ifdef _JZ_TRANSX
		if (transtemp==7) max=max-10; //7轉沒有轉前祝福
#endif
		CHAR_setInt(charaindex, CHAR_TRANSEQUATION, teq);
		print("\n重調點數[%s:%s]:%d->%d ",
    ...
    CHAR_setInt(charaindex,CHAR_SKILLUPPOINT,max);
    CHAR_complianceParameter(charaindex);
    CHAR_Skillupsend(charaindex);
    CHAR_setInt(charaindex, CHAR_TRANSMIGRATION, transtemp);
		CHAR_sendStatusString( charaindex, "P" );
    sprintf(buf,"設定轉生%d",transtemp);
		...
		for(i=0;i<getFdnum();i++){
			...
			if(i_use){
				...
				if(strcmp(token,cdkey)==0){
					...
					lv = CHAR_getInt(fd_charaindex,CHAR_LV);
			  	max=(transtemp==0)?(lv-1)*3+20+10: // 0轉447
						(lv-1)*3+table[transtemp-1]/12.0+quest/4.0+(level-transtemp*85)/4.0+10+1+10+transtemp*10;
/* 1轉以上=升級點數+繼承點術+歷史任務+歷史等級+轉前祝福+誤差1點+轉後祝福+轉生紅利 */
					if (transtemp==6) max=max-20; /* 六轉時沒有轉前祝福與轉後祝福 */
					CHAR_setInt(fd_charaindex, CHAR_TRANSEQUATION, teq);
					print("\n重調點數[%s:%s]:%d->%d ",
						CHAR_getChar(fd_charaindex,CHAR_CDKEY),
						CHAR_getChar(fd_charaindex,CHAR_NAME),
						0,max);
			 	  CHAR_setInt(fd_charaindex,CHAR_VITAL,0);
			   	CHAR_setInt(fd_charaindex,CHAR_STR,0);
			    CHAR_setInt(fd_charaindex,CHAR_TOUGH,0);
			    CHAR_setInt(fd_charaindex,CHAR_DEX,0);
			    CHAR_setInt(fd_charaindex,CHAR_SKILLUPPOINT,max);
			    CHAR_complianceParameter(fd_charaindex);
			    CHAR_Skillupsend(fd_charaindex);
			    CHAR_setInt(fd_charaindex, CHAR_TRANSMIGRATION, transtemp);
					CHAR_sendStatusString( fd_charaindex, "P" );
					sprintf(buf,"設定轉生%d",transtemp);
					CHAR_talkToCli( charaindex, -1,buf, CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_waeikick( int charindex, char* message )
{
    ...
    for(i=0;i<MAX_USER;i++){
        ...
        if(i_use){
           ...
           if(strcmp(cdkey,token)==0){
              fd_charaindex = CONNECT_getCharaindex( i );
              CONNECT_getCharname( i, szName, sizeof( szName ) );
              sprintf(kcmsg,"%s擠出服務器。",szName);
              CHAR_talkToCli(fd_charaindex, -1, "因做壞事而斷線。", CHAR_COLORYELLOW);
    ...
    if(find){
       sprintf(kctalk,"石器大魔王施展超無敵擠奶手「擠擠擠擠擠擠擠．．．．．．．把」");
       for(i=0;i<MAX_USER;i++){
           int i_use;
           i_use=CONNECT_getUse(i);
           if(i_use){
     ...
     for(i=0;i<MAX_USER;i++){
         ...
         if(i_use){
            ...
            if(strcmp(cdkey,token)==0){
               ...
               CHAR_talkToCli(fd_charaindex, -1, "因長相太差而入獄。", CHAR_COLORYELLOW);
      ...
      if(find){
         sprintf(kctalk,"石器大魔王施展超無敵抓奶手「抓抓抓抓抓抓．．．．．．．把」");
         for(i=0;i<MAX_USER;i++){
             int i_use;
             i_use=CONNECT_getUse(i);
             if(i_use){
...
void CHAR_CHAT_DEBUG_send(int charindex ,char *message)
{
     ...
     MAX_USER=getFdnum();
     if( strlen(message)==0){
        sprintf(buf,"%s","參數不正確-> FLOOR X Y 帳號");
        CHAR_talkToCli( charindex, -1,buf, CHAR_COLORWHITE);
        return;
     }
     easyGetTokenFromString( message , 1 , token, sizeof(token));
     if( strlen(token)==0){
         sprintf(buf,"%s","參數不正確-->地圖編號");
         CHAR_talkToCli( charindex, -1,buf, CHAR_COLORWHITE);
         return;
     }
     fl=atoi( token );
     ...
     for(i=0;i<MAX_USER;i++){                                                                               
         ...
         if(i_use){
            ...
            if(strcmp(token,cdkey)==0){
               ...
               snprintf( line, sizeof( line),
                         "把 name:%s account=%s 傳送到 FL=%d X=%d Y=%d",
                          szName,cdkey,fl,x,y);
                 CHAR_talkToCli( charindex, -1 , line , CHAR_COLORWHITE );
                 CHAR_talkToCli( fd_charaindex, -1 ,"＊.＊被石器大魔王傳送", CHAR_COLORWHITE );
            }                                  
         }
     }
}
...
void CHAR_CHAT_DEBUG_noenemy( int charaindex, char* message )
{
	...
	snprintf( msgbuf, sizeof( msgbuf), "不遇敵模式%s。",
		( flg == TRUE) ? "開啟" : "關閉");
...
void CHAR_CHAT_DEBUG_loginannounce( int charaindex, char* message )
{
  ...
  if( strlen(message)==0){
    sprintf(buf,"%s","參數不正確-> clear/add/send 請用小寫");
    CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
    return;
  }
...
void CHAR_CHAT_DEBUG_checklock( int charaindex, char* message )
{
  char cmd[256];
  if( strlen(message)==0){
    CHAR_talkToCli(charaindex, -1, "參數不正確", CHAR_COLORWHITE);
    return;
  }
  easyGetTokenFromString( message, 1, cmd, sizeof(cmd));
...
void CHAR_CHAT_DEBUG_fixfmdata(int charaindex, char *message)
{
  ...
  int	i,index = -1,charindex = -1,iPlayerNum = CHAR_getPlayerMaxNum();
	...
	if(index < 1 || index > 1000){
		CHAR_talkToCli(charaindex,-1,"id 值錯誤", CHAR_COLORRED);
		return;
	}
	index -= 1;
	if(szID[0] != '-' && szID[1] != '1'){
		...
		if(charindex == -1){
			CHAR_talkToCli(charaindex,-1,"找不到此帳號的人物", CHAR_COLORRED);
			return;
		}
...
void CHAR_CHAT_DEBUG_fixfmleader(int charaindex, char *message)
{
   ...
   if (strlen(message) == 0)
   {
   		CHAR_talkToCli(charaindex, -1, "參數不正確 [fixfmleader 帳號 1]", CHAR_COLORWHITE);
		return;
   }
   ...
   if(user_index == -1 ){
   		CHAR_talkToCli(charaindex, -1, "玩家不在線上!!", CHAR_COLORWHITE);	   
		return;
   }
   ...
   if( !flag ){ 
		...
		if(CHAR_getInt( user_index, CHAR_FMLEADERFLAG ) == FMMEMBER_LEADER ){
		   	CHAR_talkToCli(charaindex, -1, "玩家是其它家族的族長!!", CHAR_COLORWHITE);	   
			return;
		}
...
void CHAR_CHAT_DEBUG_manorpk(int charaindex, char *message)
{
	...
	if (message == NULL)	return;
	if( strlen(message)==0){
		sprintf(buf,"%s","參數不正確。 [manorpk <allpeace/allreset/clean> id]");
		CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
		return;
	}
	...
	if (strcmp(cmd,"allpeace")==0) {
		...
		if ((manorid<1)||(manorid>MANORNUM)) {
			sprintf(buf,"%s","參數不正確。 id 必須在 1~9 之間。");
			CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
			return;
		}
		memcpy( &tm1, localtime( (time_t *)&NowTime.tv_sec), sizeof( tm1));
		...
		sprintf(buf,"%s","參數不正確。 [manorpk <allpeace/peace> id]");
		CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
	}
}
...
void CHAR_CHAT_DEBUG_watchevent( int charaindex, char* message )
{
	...
	if( strlen(message)==0 ){
        ...
        if( strlen(token_cdkey)==0 || strlen(token_name)==0 ){
            sprintf(buf,"%s","參數不正確。正確格式：[wahctevent 帳號 人物名稱]");
            CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORWHITE);
            return;
		}
		max_user = getFdnum();
...
void CHAR_CHAT_DEBUG_gmreload( int charaindex, char* message )
{
	...
	if (strlen(message) == 0)
	{
		sprintf(buf,"%s","參數不正確，[gmreload all] 或 [gmreload id level(1~3)] level 3:工程師,2:組長級,1:一般gm");
		CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
		return;
	}
	...
	if (strcmp(id, "all") == 0)
	{
		if (charlevel != 3)
		{
			CHAR_talkToCli( charaindex, -1, "你沒有修改ＧＭ等級的權限！",  CHAR_COLORWHITE);
			return;
		}
		if (LoadGMSet(getGMSetfile()))
			CHAR_talkToCli( charaindex, -1, "再次讀取ＧＭ基本設定ＯＫ！",  CHAR_COLORWHITE);
		else 
			CHAR_talkToCli( charaindex, -1, "再次讀取ＧＭ基本設定失敗！",  CHAR_COLORRED);
	}
	else
	{
		int changeflag = 0;
		...
		{
			CHAR_talkToCli( charaindex, -1, "修改失敗，無法將此帳號等級修改的比自己等級高！level(1~3)",  CHAR_COLORWHITE);
			return;
		}
		for (i = 0; i < GMMAXNUM; i++)
		{
			if (strcmp(id, gminfo[i].cdkey) == 0)
			{
				...
				{
					snprintf(tmpbuf, sizeof(tmpbuf), "修改失敗，無法修改等級比自己高的帳號！");
					CHAR_talkToCli(charaindex, -1, tmpbuf, CHAR_COLORWHITE);
					return;
				}
				gminfo[i].level = level;
				snprintf(tmpbuf, sizeof(tmpbuf), "修改%s等級為%dＯＫ！", gminfo[i].cdkey, gminfo[i].level);
				CHAR_talkToCli(charaindex, -1, tmpbuf, CHAR_COLORWHITE);
				changeflag = 1;
				break;
			}
		...
		if (changeflag == 0)
		{
			snprintf(tmpbuf, sizeof(tmpbuf), "ＧＭ列表中查無此帳號，修改失敗！");
			CHAR_talkToCli(charaindex, -1, tmpbuf, CHAR_COLORWHITE);
		}
	}
}
...
{
#ifdef _DEATH_CONTEND				// (不可開) ANDY 死鬥
	ABATTLE_InitABattle(19);
	CHAR_talkToCli(charaindex, -1,"重讀比賽隊伍列表完成", CHAR_COLORWHITE);
#endif
}
...
void TEST_CreatPet( )
{
	...
	for( j=0; j < pet_Num; j++ )     {
		...
		CHAR_PetTakeLevelUp( petindex, pet_Lv);//強制寵物升級
		CHAR_setInt( petindex , CHAR_HP , CHAR_getWorkInt( petindex , CHAR_WORKMAXHP ) );
		...
		CHAR_PetTakeDrop( petindex, floor, -1, -1);	//強制丟棄寵物
		CHAR_setInt( petindex, CHAR_PUTPETTIME, NowTime.tv_sec);
	}
...
void CHAR_CHAT_DEBUG_cleanfloor( int charaindex, char *message)
{
	...
	sprintf( buf, "清除 寵物:%d只，道具:%d個，石幣:%d個。", petnum, itemnum, goldnum);
	CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
	return;
}
...
void CHAR_CHAT_DEBUG_setgamblenum( int charaindex, char *message )
{
	...
	sprintf( buf,"個人遊樂場積分設為%d點。", CHAR_getInt( charaindex, CHAR_GAMBLENUM));
	CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
	return;
}
...
void CHAR_CHAT_DEBUG_petlevelup( int charaindex, char* message )
{
	...
	if( (level = atoi( token)) > 140 ) return;
	ret = getStringFromIndexWithDelim( message, " ", 2, token, sizeof( token));
	if (ret){
		i = atoi( token);
		if (i<0)
			i = 0;
		...
		snprintf( msgbuf, sizeof( msgbuf ), "W%d", i );
		CHAR_sendStatusString( charaindex, msgbuf );
		snprintf( msgbuf, sizeof( msgbuf ), "寵物%d 升至%d", i,level);
		CHAR_talkToCli(charaindex, -1, msgbuf, CHAR_COLORYELLOW);
		return;
	}
	for( i=0; i<CHAR_MAXPETHAVE; i++ )	{
...
#ifdef _TRANS_6
#ifdef _JZ_TRANSX
	if( CHAR_getInt( charindex, CHAR_TRANSMIGRATION) >= _JZ_TRANSX )
#else
...
void CHAR_CHAT_DEBUG_gmkick( int charindex, char* message)
{
	...
	if( getStringFromIndexWithDelim( message, " ", 1, buf1, sizeof( buf1)) == FALSE )	{
		CHAR_talkToCli(charindex, -1, "參數不正確", CHAR_COLORYELLOW);
		return;
	}
	sprintf( cmd, "%s", buf1);
...
void CHAR_CHAT_DEBUG_getascore(int charindex, char *message)
{
	...
	char witchn[5][256]={"全","參與旗標","活動績分","戰鬥累積時間","開始時間"};
	...
	if( strstr( buf1, "clean") != 0 ){
		...
		for( i=0; i<3; i++)	{
			if( strstr( buf1, witchs[i]) != 0 ){
				char buf2[256];
				int nums=0;
				char witchn[3][256]={"參與旗標","活動績分","開始時間"};
				if( getStringFromIndexWithDelim( message, " ", 2, buf2, sizeof( buf2)) == FALSE )
					return;
				nums = atoi( buf2);
				if( i == 2 )	{
					sprintf( buf2, "%s設定為提前%d。", witchn[i], nums);
					nums = (nums * 24) * (60 * 60);
					nums = (int)time( NULL) - nums;
				}else{
					sprintf( buf2, "%s設定為%d。", witchn[i], nums);
				}
...
void CHAR_CHAT_DEBUG_saveditem(int charaindex, char *message)
{
	if( CHAR_SaveDepotItem( charaindex) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "儲存失敗！", CHAR_COLORYELLOW);
	}
}
...
void CHAR_CHAT_DEBUG_insertditem(int charaindex, char *message)
{
	if( CHAR_GetDepotItem( -1, charaindex) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "讀取失敗！", CHAR_COLORYELLOW);
	}
}
...
{	
	...
	if( delobjnum > 0 ){
		char buf[256];
		sprintf( buf, "清除%d  置寵物。", delobjnum);
...
{	
	...
	if( RunType < 1 || RunType > 2 ){
		CHAR_talkToCli( charaindex, -1, "參數錯誤", CHAR_COLORRED);
		return;
	}
...
//	newsec = localtime( &newsec);
	...
	newsec = mktime( p);
	sprintf( buf, "確認1：%d年%d月%d日 %s %d:%d:%d ...newsec:%ld。",
		(1900+p->tm_year), (1+p->tm_mon), p->tm_mday, WKday[p->tm_wday],
		p->tm_hour, p->tm_min, p->tm_sec, newsec );
	CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORRED);

	p = localtime( &newsec);
	sprintf( buf, "確認2：%d年%d月%d日 %s %d:%d:%d ...newsec:%ld..time:%d。",
		(1900+p->tm_year), (1+p->tm_mon), p->tm_mday, WKday[p->tm_wday],
...
void CHAR_CHAT_DEBUG_cleanqute( int charaindex, char *message)
{
	...
	for( i=0; i<playernum; i++ ){
		...
		if( !strcmp( CHAR_getChar( i, CHAR_CDKEY), cdkey) &&
			...
			sprintf( token,"清除%s活動記號。", CHAR_getUseName(i) );
			CHAR_talkToCli( charaindex, -1, token, CHAR_COLORRED);
			sprintf( token,"%s清除你的活動記號。", CHAR_getUseName(charaindex) );
			CHAR_talkToCli( i, -1, token, CHAR_COLORRED);
			return;
		}
	}
	CHAR_talkToCli( charaindex, -1, "沒有找到對像!!", CHAR_COLORRED);
}
#endif
...
void CHAR_CHAT_DEBUG_show_profession( int charaindex, char*message )
{
	...
	if( strcmp(type, "restart") == 0 ){	// 重讀 profession.txt 
			CHAR_talkToCli( charaindex, -1, "重讀 profession.txt", CHAR_COLORRED);
			rePROFESSION_initSkill();
	}else if( strcmp(type, "") != 0 ){	// 修改職業屬性
		if( strcmp(worker, "") != 0 ){
			if( strcmp(value, "") != 0 ){
				...
				CHAR_talkToCli( charaindex, -1, "請輸入數值", CHAR_COLORRED);
		}else 
			CHAR_talkToCli( charaindex, -1, "請輸入修改項目：1(職業別) 2(職業等級) 3(職業經驗值) 4(剩餘點數)", CHAR_COLORRED);
	}

	if(  strcmp(type, "1") != 0 ){
		memset(msg, 0, sizeof(msg));
		sprintf(msg, "職業別(%d) 職業等級(%d) 剩餘點數(%d)",
...
{
    int i;
	int MAX_USER=getFdnum(); //玩家數量
    CHAR_setChar(charaindex, CHAR_GMIDENTIFY, message); //設定gm字串
    CHAR_sendCToArroundCharacter(CHAR_getWorkInt( charaindex, CHAR_WORKOBJINDEX )); //傳送給周圍的人
	//for(i=0;i<MAX_USER;i++){
...
void CHAR_CHAT_DEBUG_updatepklist( int charaindex, char* message )
{
	...
	if( PKLIST_GetTeamLeaderCdkey( mynum, mycdkey) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "無法找到隊伍1", CHAR_COLORYELLOW);
		return;
	}
	if( PKLIST_GetTeamLeaderCdkey( tonum, tocdkey) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "無法找到隊伍2", CHAR_COLORYELLOW);
		return;
	}
...
void CHAR_CHAR_DEBUG_reloadfm( int charaindex, char* message )
{
	...
	CHAR_talkToCli( charaindex, -1, "重讀家族資料中...", CHAR_COLORYELLOW);		
...
void CHAR_CHAR_DEBUG_fmpk( int charaindex, char* message )
{
	...
	if( getStringFromIndexWithDelim( message, " ", 1, buf1, sizeof(buf1) ) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "[fmpk 家族1 家族2 時間 場地]",  CHAR_COLORYELLOW);
		return;
	}
	fm1 = atoi( buf1);
	if( getStringFromIndexWithDelim( message, " ", 2, buf1, sizeof(buf1) ) == FALSE ) return;
...
void CHAR_CHAR_DEBUG_fmpk_clean( int charaindex, char* message )
{
	...
	if( getStringFromIndexWithDelim( message, " ", 1, buf1, sizeof(buf1) ) == FALSE ){
		CHAR_talkToCli( charaindex, -1, "[fmpk_clean 場地 時間]",  CHAR_COLORYELLOW);
		return;
	}
	id = atoi( buf1 );
	...
	for( i=0; i<MAX_SCHEDULE; i++){		
		if( fmpks[fmpks_pos+i].dueltime == hour*100 || hour == 25){
			...
			sprintf( msg, "ID:%d TIME:%d 家族挑戰清除完成", id, time );
			CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW );
...
void CHAR_CHAT_DEBUG_set_momentum( int charaindex, char* message )
{
	...
	if(i >= iPlayerNum){
		CHAR_talkToCli(charaindex,-1,"找不到這名玩家",CHAR_COLORYELLOW);
		return;
	}
	CHAR_setInt(i,CHAR_MOMENTUM,id);
	sprintf(szMsg,"目前氣勢值: %d",CHAR_getInt(i,CHAR_MOMENTUM)/100);
...
void CHAR_CHAT_DEBUG_set_manor_owner( int charindex, char* message )
{
	...
	if(message == NULL || getStringFromIndexWithDelim(message," ",1,szId,sizeof(szId)) == FALSE){
		CHAR_talkToCli(charindex,-1, "參數不正確 [set_manor_owner 莊園id(1~9) 使用旗標(0:名稱 1:id) 家族名稱或id]", CHAR_COLORRED);
		return;
	}
	if(atoi(szId) < 1 || atoi(szId) > MANORNUM){
		CHAR_talkToCli(charindex,-1, "莊園ID值不正確(1~9)", CHAR_COLORRED);
		return;
	}
	...
	if(iUseFlag < 0 || iUseFlag > 1){
		CHAR_talkToCli(charindex,-1, "使用旗標值不正確(0:名稱 1:id)", CHAR_COLORRED);
		return;
	}
...
void CHAR_CHAT_DEBUG_set_schedule_time( int charindex, char* message )
{
	...
	if(message == NULL || getStringFromIndexWithDelim(message," ",1,szId,sizeof(szId)) == FALSE){
		CHAR_talkToCli(charindex,-1, "參數不正確 [set_schedule_time 莊園id(1~9) 時間(分)]", CHAR_COLORRED);
		return;
	}
	if(atoi(szId) < 1 || atoi(szId) > MANORNUM){
		CHAR_talkToCli(charindex,-1, "莊園ID值不正確", CHAR_COLORRED);
		return;
	}
	...
	for(i=0;i<char_max;i++){
		if(CHAR_getCharUse(i) && (CHAR_getInt(i,CHAR_WHICHTYPE) == CHAR_TYPEMANORSCHEDULEMAN)){
			if(CHAR_getWorkInt(i,CHAR_NPCWORKINT2) == atoi(szId)){
				...
				sprintf(szMsg,"莊園時間設定為 %s 分",szTime);
				CHAR_talkToCli(charindex,-1,szMsg,CHAR_COLORRED);
				return;
			}
		}
	}
	CHAR_talkToCli(charindex,-1,"找不到莊園npc",CHAR_COLORRED);
}
...
#ifdef _ANGEL_SUMMON
void CHAR_CHAT_DEBUG_angelinfo(int charindex ,char *message) // 顯示Mission資料
{
	...
	if( message[0] == NULL || getStringFromIndexWithDelim( message, " ", 1, cdkey, sizeof(cdkey) ) == FALSE ){
		CHAR_talkToCli( charindex, -1, "參數不正確 [angelinfo 帳號(all) (clean)]", CHAR_COLORRED);
		return;
	}
	if( getStringFromIndexWithDelim( message, " ", 2, buf, sizeof(buf) ) != FALSE ){
	...
	CHAR_talkToCli( charindex, -1, "....結束", CHAR_COLORYELLOW);
}

void CHAR_CHAT_DEBUG_angelclean(int charindex ,char *message) // 清除Mission資料
{
	int mindex;
	char msg[1024];
	...
	if( mindex <0 || mindex >=MAXMISSIONTABLE) {
		CHAR_talkToCli( charindex, -1, "編號不正確", CHAR_COLORYELLOW);
		return;
	}
	if( missiontable[mindex].flag == MISSION_NONE ) {
		CHAR_talkToCli( charindex, -1, "無此資料", CHAR_COLORYELLOW);
		return;
	}
	
	saacproto_ACMissionTable_send( acfd, mindex, 3, "", "");

	sprintf( msg, " GM指令 清除任務資料 i:%d 使者:%s 勇者:%s m:%d flg:%d time:%d lt:%d ",
...
void CHAR_CHAT_DEBUG_angelcreate(int charindex ,char *message) // 製造使者與勇者配對
{
	...
	if( getStringFromIndexWithDelim( message, " ", 1, angelid, sizeof(angelid) ) == FALSE ){
		CHAR_talkToCli( charindex, -1, "參數不正確 [angelcreate 使者帳號 勇者帳號 任務編號]", CHAR_COLORRED);
		return;
	}
	...
	if( mission >= MAXMISSION ) {
		CHAR_talkToCli( charindex, -1, "任務編號不正確", CHAR_COLORRED);
		return;
	}
...
void CHAR_CHAT_DEBUG_missionreload(int charindex ,char *message)
{
	...
	CHAR_talkToCli( charindex, -1, "重新讀取使者任務列表...", CHAR_COLORYELLOW);
	if( !LoadMissionList( ) ) {
		CHAR_talkToCli( charindex, -1, "讀取失敗", CHAR_COLORYELLOW);
		print("fail!!\n");
		return;
	}
	print("done\n");
	CHAR_talkToCli( charindex, -1, "讀取完成", CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_itemreload(int charindex ,char *message)
{
	...
	CHAR_talkToCli( charindex, -1, "重新道具資料...", CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_skywalker(int charaindex ,char *message)
{
	...
	if( strstr( message, "on") != NULL )	{
		CHAR_talkToCli( charaindex, -1, "天行者模式啟動", CHAR_COLORYELLOW);
		CHAR_setWorkInt( charaindex, CHAR_WORKSKYWALKER, TRUE);
	}
	if( strstr( message, "off") != NULL )	{
		CHAR_talkToCli( charaindex, -1, "天行者模式關閉", CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_itemaddexp(int charaindex ,char *message)
{
	...
	if( !strcmp( message, "") ) {
		...
		if( toindex == -1 ) {
			CHAR_talkToCli( charaindex,-1, "查無此人", CHAR_COLORRED);
			return;
		}
	}

	sprintf( msg, "智慧之果效果  %s　目前分數%d  %d％  時間%d分。",
...
void CHAR_CHAT_DEBUG_fmfreetime(int charaindex,char *message)
{
	...
	if( strlen(token_cdkey)==0){
        sprintf(buf,"%s","參數不正確。正確格式：[fmfreetime  帳號]");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
	for( i=0;i<Playernum;i++ ){
...
void CHAR_CHAT_DEBUG_getyou(int charaindex,char *message)
{
	...
	area = atoi(areabuf);
	if( area <= 0 || area > 3 ){
        sprintf(buf,"%s","沒有輸入範圍或範圍超出限制，請輸入1~3");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
		...
		for(j=x-area;j<=x+area;j++){
			for( object = MAP_getTopObj(floor,j,i) ; object ;object = NEXT_OBJECT(object ) ){
				...
				if( OBJECT_getType(o) == OBJTYPE_CHARA ){
					...
					if( ( fd = getfdFromCharaIndex(chara_index) ) != -1 ){				
						getnum++;
						CONNECT_getCharname( fd, szName, sizeof( szName ) );  //人物名稱
						CONNECT_getCdkey( fd, cdkey, sizeof( cdkey ));		  //帳號	
						//CHAR_getChar( chara_index, CHAR_NAME);  
						snprintf( line, sizeof( line), "第%d個找到[%s]的帳號是[%s]",getnum,szName,cdkey);
                        CHAR_talkToCli( charaindex, -1,line, CHAR_COLORRED);
						print("第%d個找到[%s]的帳號是[%s]\n",getnum,szName,cdkey);					
					}
				}
			}
		}
	}
	if(!getnum) CHAR_talkToCli( charaindex, -1, "  圍內沒有人", CHAR_COLORYELLOW);
}
#endif
...
void CHAR_CHAT_DEBUG_newsend(int charindex ,char *message)
{
	...
	MAX_USER=getFdnum();
	if( strlen(message)==0){
		sprintf(buf,"%s","參數不正確-> FLOOR X Y 無/要說的話");
		CHAR_talkToCli( charindex, -1,buf, CHAR_COLORWHITE);
		return;
	}
	easyGetTokenFromString( message , 1 , token, sizeof(token));
	...
	for(i=0;i<MAX_USER;i++){                                                                               
		...
		if(i_use){
			...
			if(strcmp(token,cdkey)==0){
				...
				CHAR_warpToSpecificPoint(fd_charaindex, fl, x, y);
				snprintf( line, sizeof( line),
						 "把 name:%s account=%s 傳送到 FL=%d X=%d Y=%d say:%s",
...
void CHAR_CHAT_DEBUG_supersend(int charaindex ,char *message)
{
    ...
    OBJECT  object;
	int fl,x,y,fd,fd_charaindex; //帳號所在位置
	unsigned int MAX_USER=0;
	int sendnum = 0;
	int o,chara_index;
	...
	if( strlen(message)==0){
		sprintf(buf,"%s","參數不正確-> FLOOR X Y 帳號 格數 無/要說的話");
		CHAR_talkToCli( charaindex, -1,buf, CHAR_COLORWHITE);
		return;
	}
	easyGetTokenFromString( message , 1 , token, sizeof(token));
	...
	if( strlen(token)==0 ||	area <= 0 || area > 3 ){
		sprintf(buf,"%s","參數不正確-->格數(1~3)");
		CHAR_talkToCli( charaindex, -1,buf, CHAR_COLORWHITE);
		return;
	}
	easyGetTokenFromString( message , 6 , say , sizeof(say));  //說話
    ...
    for(k=0;k<MAX_USER;k++){
        ...
        if(i_use){
            ...
            if(strcmp(cdkey,cdkeytoken)==0){
							...
							if( OBJECT_getType(o) == OBJTYPE_CHARA ){
								...
								if( CHAR_getInt( chara_index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
									...
									if( ( fd = getfdFromCharaIndex(chara_index) ) != -1 ){				
										...
										snprintf(line,sizeof( line),"第%d個傳送[%s]",sendnum,szName);
				...
				if(sendnum<=1) CHAR_talkToCli( charaindex, -1, "  圍內沒有別的玩家", CHAR_COLORYELLOW);
				snprintf( line, sizeof( line),"把 name:%s account=%s 周圍%d格內的玩家共%d個傳送到 FL=%d X=%d Y=%d",
...
void CHAR_CHAT_DEBUG_fsize( int charaindex, char* message )
{
	...
	//CHAR_send_P_StatusString( charaindex , CHAR_P_STRING_LV);
	sprintf( buf1, "說話字型大小設為%d。", atoi( message ));
	CHAR_talkToCli( charaindex, -1, buf1, CHAR_COLORYELLOW);
}
#endif
...
{
	CHAR_talkToCli( charindex, -1, "重新讀取自定義騎寵列表...", CHAR_COLORYELLOW);
	print("GM Reading Ride File...");
...
void CHAR_CHAT_DEBUG_rejobdaily(int charaindex ,char *message)
{
	...
	CHAR_talkToCli( charaindex, -1, "重新讀取任務日誌列表...", CHAR_COLORYELLOW);
	if(!LoadJobdailyfile()){
		CHAR_talkToCli( charaindex, -1, "讀取失敗", CHAR_COLORYELLOW);
		print("fail!!\n");
		return;
	...
	print("done\n");
	CHAR_talkToCli( charaindex, -1, "讀取完成", CHAR_COLORYELLOW);
}
#endif
...
void CHAR_CHAT_DEBUG_changeask(int charaindex, char *message)
{
	...
	if( strlen(cdkey)==0){
        sprintf(buf,"%s","沒有帳號。正確格式：[changeask  帳號  人物名稱  寵證id 0/1]");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
	easyGetTokenFromString( message , 2 , item, sizeof(item));
	if( strlen(item)==0){
        sprintf(buf,"%s","沒有寵證id。正確格式：[changeask  帳號  寵證id 0/1]");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
	easyGetTokenFromString( message , 3 , free , sizeof(free));
	if( strlen(free)==0 ){
        sprintf(buf,"%s","最後的參數錯誤。正確格式：[changeask  帳號 寵證id 0/1]");
        CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
        return;
	}
		...
		if( ITEM_CHECKINDEX( itemindex) ) {
			...
			if(itemid==atoi(item)){	//找到獵寵證
				...
				if(CHAR_getInt( toindex,CHAR_CATCHCNT5 ) >= asktotal ){
					sprintf( buf,"恭喜你達到獵寵大賽終極目標，祝您抽中大獎");
				}else{
					...
					if( asktable[askno].highlv == 0 )
						sprintf( buf,"請抓給我等級%d『以上』的%s好嗎。您目前記錄%d種", asktable[askno].lowlv , asktable[askno].petname, CHAR_getInt( toindex,CHAR_CATCHCNT5 ) );
					else
						sprintf( buf,"請抓給我『只能』等級%d的%s好嗎。您目前記錄%d種", asklv , asktable[askno].petname, CHAR_getInt( toindex,CHAR_CATCHCNT5 ) );
				}				
...
void CHAR_CHAT_DEBUG_BestMM(int charindex ,char *message)
{
	...
	if(atoi(message)== 1 ){
		...
		CHAR_talkToCli( charindex, -1, "參數錯誤->1/2 （1=1代MM 2=2代MM）", CHAR_COLORYELLOW);
	}
	enemynum = ENEMY_getEnemyNum();
	for( i = 0; i < enemynum; i ++ ) {
		if( ENEMY_getInt( i, ENEMY_ID) == enemyid) {
	...
	if( !CHAR_CHECKINDEX( ret))return;
	snprintf( msgbuf,sizeof( msgbuf), "極品MM製作成功\!index[%d]",ret);
...
void CHAR_CHAT_DEBUG_Support(int charindex ,char *message)
{
	...
	lssproto_Shutdown_recv(0, "hogehoge",timemintemp);	// n分鐘後維修
}
...
#ifdef _JZ_TRANSX
		if( transtemp > _JZ_TRANSX)transtemp=_JZ_TRANSX;
#else
...
#endif
#ifdef _JZ_CAPTURE_FREES
void CHAR_CHAT_DEBUG_recapfree(int charindex ,char *message)
{
	CHAR_talkToCli( charindex, -1,"開始重讀capfree.....", CHAR_COLORWHITE);
	if(BATTLE_InitCapFree(getCapFreeFile()))
		{
			CHAR_talkToCli( charindex, -1,"成功！", CHAR_COLORWHITE);
			return;
		}
		CHAR_talkToCli( charindex, -1,"失敗！", CHAR_COLORWHITE);
}
#endif
...
void CHAR_CHAT_DEBUG_charfame( int charaindex, char *message)
{
	...
	char charkey[32];
	int buff1[32];
	if(getStringFromIndexWithDelim( message, " ", 2, charkey , sizeof( charkey ))) //判斷參數個數
	{
		charfame = atoi(charkey);
		getStringFromIndexWithDelim( message, " ", 1, charkey , sizeof( charkey )); //帳號
		for(i=0;i<getFdnum();i++){
			int i_use,fd_charindex;
			char cdkey[64];
			i_use=CONNECT_getUse(i);
			if(i_use){
				CONNECT_getCdkey( i, cdkey, sizeof( cdkey ));
				if(strcmp(charkey,cdkey)==0){
					fd_charindex = CONNECT_getCharaindex( i );
					CHAR_setInt(fd_charindex, CHAR_FAME, charfame);
					sprintf(buff1,"設置聲帳號:%s 望值為:%d",charkey,charfame/100);
					CHAR_talkToCli( fd_charindex, -1,buff1, CHAR_COLORWHITE);
					return;
				}
			}
		}
	}
	getStringFromIndexWithDelim( message, " ", 1, charkey , sizeof( charkey ));
	charfame = atoi(charkey);
	CHAR_setInt(charaindex, CHAR_FAME, charfame);
	sprintf(buff1,"設置聲望值為:%d",charfame/100);
	CHAR_talkToCli( charaindex, -1,buff1, CHAR_COLORWHITE);
}

#ifdef _JZ_CIRC_AN
void CHAR_CHAT_DEBUG_reloopan( int charaindex, char *message)
{
...
#ifdef _JZ_LOCKIP
void CHAR_CHAT_DEBUG_showip( int charaindex, char *message)
{
	char charip[64];
	char cdkey[64];
	char buff1[128];
	...
	for(i=0;i<getFdnum();i++){
		...
		if(i_use){
			CONNECT_getCdkey( i, cdkey, sizeof( cdkey ));
			if(strcmp(message,cdkey)==0){
				net_showip(i,charip);
				sprintf(buff1,"玩家:%s的IP地址:%s",message,charip);
				CHAR_talkToCli( charaindex, -1,buff1, CHAR_COLORWHITE);
			}
		}
	}
...
void CHAR_CHAT_DEBUG_sendto(int charindex ,char *message)
{
	char szName[256];
	char cdkey[CDKEYLEN];
	char line[256];
