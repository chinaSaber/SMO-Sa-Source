...
BOOL PKLIST_CheckPKSameTeam( int charaindex )
{
	...
	for( i=0; i<5; i++){
		...
		if( teamnum != CHAR_getInt( pindex, CHAR_PKLISTTEAMNUM) ){
			CHAR_talkToCli( charaindex, -1, "你的隊裡怎會有不同隊伍的人呢？", CHAR_COLORYELLOW);
			return FALSE;
		}
	}
	for( i=0; i<5; i++){
		...
		if(CHAR_getInt(pindex,CHAR_LV) < 120){
			sprintf(szMsg,"你的隊裡 %s 等級 %d 未達比賽標準喔！",CHAR_getChar(pindex,CHAR_NAME),CHAR_getInt(pindex,CHAR_LV));
			CHAR_talkToCli(charaindex,-1,szMsg,CHAR_COLORYELLOW);
			return FALSE;
		}
		if(CHAR_getInt(pindex,CHAR_TRANSMIGRATION) < 3){
			sprintf(szMsg,"你的隊裡 %s 轉生數 %d 轉，未達比賽標準喔！",CHAR_getChar(pindex,CHAR_NAME),CHAR_getInt(pindex,CHAR_TRANSMIGRATION));
			CHAR_talkToCli(charaindex,-1,szMsg,CHAR_COLORYELLOW);
			return FALSE;
		}
	}
...
void PKLIST_warp( int ti, int side, int fl, int x, int y )
{
	...
	if( CHAR_getWorkInt( PKProcedure[ti].Team[side].toindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_LEADER ){
		...
		for( k = 0; k < CHAR_PARTYMAX; k ++ ){
			...
			if( !CHAR_CHECKINDEX( subindex ) )continue;
			CHAR_talkToCli( subindex, -1, "請在此等待對戰隊伍！如時間超過１０分後未有對戰隊伍，建議玩家可重原登後重新加入賽程。", CHAR_COLORYELLOW);
			CHAR_warpToSpecificPoint( subindex, fl, x, y);
		}
	}
}
...
BOOL ABATTLE_InsertBattle( ArrangeBattleC *aB)
{
	...
	for( i=0; i<MAXBATTLEPAGE; i++){
		...
		aB->time = (int)time(NULL)+30;	//戰鬥時間
#else
		aB->time = (int)time(NULL)+(5*60);	//戰鬥時間
#endif
		aB->type = 1; // 戰鬥旗標
		return TRUE;
	}
		...
		for( j=0; j<2; j++){
			...
			if( aB2 == NULL || aB2->use == 0 || aB2->teamnum == -1 ){ //沒有對手
				...
				if( ABATTLE_InsertBattle( aBo) == FALSE ){
					...
					char token[256];
					int playernum = CHAR_getPlayerMaxNum();
					sprintf( token, "%s隊 VS %s隊 ５分鐘後決鬥。", aB1->teamname, aB2->teamname );
	...
	//for( i=0; i<MAXBAHEAD; i++){
	{
		...
		if( ABFloor[i] == -1 ){
			...
			if( headABattle[i]->use == 1 && headABattle[i]->teamnum != -1 ){
				...
				ABATTLE_RecordBattle( i, headABattle[i]->teamname, "晉級８強", "NULL", "");
...
BOOL ABATTLE_CheckInABattle( int ti)
{
	...
	case 1:	//已排入賽程 5分鐘後進場
		...
		break;
	case 2:	//已排入賽程 進場3分鐘 並判斷隊長是否存在 並廣播
		{
			int playernum = CHAR_getPlayerMaxNum();
			for( i=0 ; i<playernum ; i++) {
				if( CHAR_getCharUse(i) == FALSE ) continue;
				if( aB1!=NULL && !CHAR_CHECKINDEX( aB1->toindex) &&
					...
					PKLIST_CheckPKSameTeam( i) == TRUE ){
						...
						sprintf( token, "請快找齊你的所有隊員，並組好隊，%d秒後將進行傳送。", (stime<0)?0:stime);
						CHAR_talkToCli( i, -1, token, CHAR_COLORYELLOW);
						aB1->toindex = i;
						continue;
		...
		break;
	case 3:	//已排入賽程 隊伍到齊 5分鐘內 強制戰鬥
		aB->type=4;
		aB->time=(int)time(NULL)+(2*60);
		break;
	case 4:
		if( aB->time < (int)time(NULL) ){//強制戰鬥
