...
BOOL NPC_CheckJoin_FamilyPK_List( int meindex, int toindex)
{
	...
	print( "check[%d]: [%d,%s,%s]\n",
		...
		if( JoinFamilyList[i].join == 1 ){
			CHAR_talkToCli( toindex, -1, "已經報名參加了。", CHAR_COLORYELLOW);
			return FALSE;
		}else{
			CHAR_talkToCli( toindex, -1, "報名完成。", CHAR_COLORYELLOW);
			JoinFamilyList[i].join = 1;
			NPC_BackupFamilyPK_List();
			return TRUE;
		}
	}
	CHAR_talkToCli( toindex, -1, "無法報名，請確認你是否為資格名單上之家族族長。", CHAR_COLORYELLOW);
	return FALSE;
}
//CHAR_FMINDEX 
#endif
