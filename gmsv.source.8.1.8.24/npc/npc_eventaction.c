...
BOOL Action_RunDoEventAction( int meindex, int toindex, char *buf1)
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "ABULLSCORE", buf2, sizeof( buf2) ) != NULL) {
		if( strstr( buf2, "TRUE") != NULL ){
			if( CHAR_getInt( toindex, CHAR_ABULLSTART) != 20 ){
				...
				CHAR_talkToCli( toindex, -1, "你開始參予與此活動。",  CHAR_COLORYELLOW);
			}else {
				return FALSE;
			}
		}else	{
	...
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "CHECKSCORE", buf2, sizeof( buf2) ) != NULL) {
		if( strstr( buf2, "TRUE") != 0 ){
			if( CHAR_getInt( toindex, CHAR_ABULLSTART) != 20 &&
				...
				if( ntime < 0 ) ntime = 0;
				sprintf( buf1, "戰鬥時間已累積%d分%d秒，活動累積積分：%d， 參與%d天%d小時。",
	...
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "PROFESSION", buf2, sizeof( buf2) ) != NULL) {
		...
		if( p_class > PROFESSION_CLASS_NONE && p_class < PROFESSION_CLASS_NUM ) //	1:戰士 2:巫師 3:獵人
		{
			int i, value = 0;
			int skill_id = -1;
			...
			switch( p_class ){
				...
				case PROFESSION_CLASS_WIZARD:	value = 10;	skill_id = 1;  break;	// 巫師
				case PROFESSION_CLASS_HUNTER:	value = 25;	skill_id = 47; break;	// 獵人
				default: break;
			}
/*
			// 給第一個職業技能
...
#endif
#ifdef _JZ_VIPLOG
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "AddMemberItem", buf2, sizeof( buf2)) != NULL ) {
		if( NPC_ActionMemberAddItem( toindex, buf2) == FALSE )
			return FALSE;
	}
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "AddMemberPet", buf2, sizeof( buf2)) != NULL ) {
		if( NPC_ActionMemberAddPet( toindex, buf2) == FALSE )
			return FALSE;
	}
#endif
#ifdef _JZ_NEW_ADDMEMBERPOINT
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "BMM", buf2, sizeof( buf2)) != NULL ) {
		int randMM=0;
		while(1){
			if(getStringFromIndexWithDelim( buf2, ",", randMM, buf3,sizeof( buf3))){
				randMM++;
			}else{
				break;
			}
		...
		print("BMM Test:1");
		if( NPC_EventBMM( -1,toindex, buf2,randMM) == FALSE )
			return FALSE;
	}
...
BOOL NPC_ActionDelGold( int talker,char *buf)
{
	...
	if( PGold < Golds )	{
		CHAR_talkToCli( talker, -1, "你身上的錢不夠喔！", CHAR_COLORYELLOW);
		return FALSE;
	}else	{
		char buf1[256];
...
BOOL NPC_ActionDelItem( int talker, char *buf)
{
	...
	while( getStringFromIndexWithDelim(buf , "," , k, buff3, sizeof(buff3)) !=FALSE )	{
		...
		if(strstr(buff3,"*")!=NULL){
			...
			if(cnt != kosuu){
				CHAR_talkToCli( talker, -1,"條件不足", CHAR_COLORYELLOW);
				return FALSE;
			}
			//收取物品
			for( i=0; i<20; i++)	{
  ...
  while( getStringFromIndexWithDelim(buf , "," , k, buff3, sizeof(buff3)) !=FALSE ){
	k++;
		...
		if( i == CHAR_MAXITEMHAVE )	{
			snprintf( msgbuf,sizeof( msgbuf), "物品欄空間不足！！");
			CHAR_talkToCli( talker, -1, msgbuf,  CHAR_COLORYELLOW);
			return FALSE;
		}
  }
...
BOOL NPC_ActionWarpManReduce2(int meindex,int talker,char *buf)
{
	...
	for( i=CHAR_STARTITEMARRAY ; i < CHAR_MAXITEMHAVE;i++ ){
		...
		if( ITEM_CHECKINDEX(itemindex) ){
			id=ITEM_getInt(itemindex ,ITEM_ID );
			if( itemno == id ){
				nums += ITEM_getInt( itemindex, ITEM_USEPILENUMS);//數量
			}
		}
	}
...
{
	int NewMember=0;
	int OldMember,OldMember1;
	char token[256];
	NewMember = atoi( buf);
	OldMember1 = CHAR_getInt(talker, CHAR_MEMBERORDER);
	if (OldMember1<NewMember)return FALSE;
	OldMember = OldMember1 - NewMember;
...
#endif
	sprintf( token, "會員點數減少:%d。剩餘點數:%d。", NewMember,OldMember);
	CHAR_talkToCli( talker, -1, token, CHAR_COLORYELLOW);
	return TRUE;
}
...
BOOL NPC_ActionAddProfessionSkillPoint( int toindex, char *buf )
{
	...
	CHAR_sendCToArroundCharacter( CHAR_getWorkInt( toindex , CHAR_WORKOBJINDEX ));
	sprintf( token, "技能學習點數增加%d點。", addsp);
	CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW);
	return TRUE;
}
...
#ifdef _FIXBUG_ATTACKBOW
	{
		...
		if( ITEM_CHECKINDEX(itemindex) ){
			...
			if( armtype == ITEM_BOW || armtype == ITEM_BOUNDTHROW ||
				...
				CHAR_talkToCli( charindex, -1, "使用遠距離武器無法變身！", CHAR_COLORYELLOW );
				return FALSE;
			}
		}
...
#endif
	if( CHAR_getInt( charindex, CHAR_RIDEPET) != -1 )	{//騎寵不換圖
		CHAR_setInt( charindex, CHAR_RIDEPET, -1 );
		BATTLE_changeRideImage( charindex );
		CHAR_talkToCli( charindex, -1, "取消騎寵狀態。", CHAR_COLORYELLOW);
		
		//change -----> 修改bug(落馬後,寵物選項的狀態依然為"騎乘",這裡修正過來 
...
int NPC_DischargePartyForSex( int meindex, int charaindex, int fl, int x, int y, int fsex)
{
	...
	CHAR_talkToCli( charaindex, -1, "團隊重組！", CHAR_COLORYELLOW);
...
BOOL GMQUE_getQueStr( int meindex, int charaindex)
{
	if( CHAR_getInt( charaindex, CHAR_GMQUEFLG) == 10 ){
		CHAR_talkToCli( charaindex, -1, "已參加抓寵活動。", CHAR_COLORYELLOW);
		return FALSE;
	}

	if( GMQUE_InSertQue( meindex, charaindex) == FALSE ) return FALSE;
	
	GMQUE_showQueStr( meindex, charaindex);//秀訊息
	return TRUE;
}

void GMQUE_cleanQueStr( int nindex)
{
	CHAR_talkToCli( nindex, -1, "活動任務結束。", CHAR_COLORYELLOW);
...
void GMQUE_showQueStr( int meindex, int charaindex)
{
	...
	sprintf( token,"抓寵任務目標：\n");
	if( CHAR_getInt( charaindex, CHAR_GMQUEFLG) != 10 ){
		CHAR_talkToCli( charaindex, -1, "尚未參加抓寵活動。", CHAR_COLORYELLOW);
		return;
	}
...
//		petTempNo = ENEMY_getEnemyTempNoFromId( atoi( buf2));
		...
		if( j >= CHAR_MAXPETHAVE ){
			snprintf( buf1,sizeof( buf1), "人物身上並沒有%s LV:%d。", ENEMYTEMP_getChar( array, E_T_NAME), lvarr);
	...
	if( CHAR_getInt( charaindex, CHAR_GMQUENUMS) > 97 ){//pet
		...
		if( CHAR_findEmptyItemBox( charaindex ) == -1 ){
			CHAR_talkToCli( charaindex, -1, "身上物品欄位已滿，無法領獎。", CHAR_COLORYELLOW);
			return FALSE;
		}
	}else {//gold
		if( CHAR_getInt( charaindex, CHAR_GOLD) >= 800000 ){
			CHAR_talkToCli( charaindex, -1, "身上石幣大於等於80萬，無法領獎。", CHAR_COLORYELLOW);
			return FALSE;
		}
	}

	if( count != petset ){
		sprintf( buf1,"找到%d只符合條件的寵，但需要%d只。", petset, count);
		CHAR_talkToCli( charaindex, -1, buf1, CHAR_COLORYELLOW);
		return FALSE;
	}
	return TRUE;
...
BOOL NPC_ActionCheckABullEvent( int meindex, int talker)
{
	...
	"夢德洞窟任務", "馬祖任務", "強恩一族", "龍穴任務", "夢幻洞窟", "夢幻洞窟", "夢幻洞窟",
	"黃金羚羊", "五兄弟之謎", "四聖石任務", "金飛任務", "火靈勇士任務" };
...
/*	大點視為浮點數 浮點數會誤判 1950 + 50 = 2000 但轉換後/100 = 19 + 0 = 19
...
*/	
	...
	CHAR_setInt( charaindex, CHAR_VITAL, vital *100);	// 體
	...
	snprintf( msg, sizeof(msg), "還原點數得到 %d", MySp );
...
#endif
#ifdef _JZ_VIPLOG
BOOL NPC_ActionMemberAddItem( int talker, char *buf)
{
  int itemID,k=0,itemindex=-1;
  int spaceNum=5,i;
  char buff3[256], msgbuf[64], token[256];
...
BOOL NPC_ActionMemberAddPet( int talker, char *buf)
{
	int	ret;
	char msgbuf[64], buff3[256];
	int	enemynum;
