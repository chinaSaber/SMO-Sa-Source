...
enum {
	...
	NPC_WORK_WORKTYPE = CHAR_NPCWORKINT2,	// 0,-1 = NULL 2 = 銀行 3 = 換幣 4 = BOTH
...
#define _GAMBLEBANK_U_NOLOCK	//同時對多人

#define GAMBLEBANK_LOOPTIME 80
#define GAMBLEBANK_STANDBY	5000 
#define GAMBLEBANK_DEF		3	//手續費 / 100
...
static void NPC_GambleBank_selectWindow( int meindex, int toindex, int num, int flg)
{
	...
	int i;
	int page=-1;
	//flg <= 0 錯誤 1 正常 flg = 2 錢不夠  3 = 個人存款將滿
	//		4 = 個人銀行將滿 5 = 提領上限 6 = 提領下限
	char Gamble_End[][56] = {
...
void NPC_GambleBankWindowTalked ( int meindex, int talkerindex, int seqno, int select, char *data)
{
	...
	switch( seqno) {
	  ...
	  case NPC_GambleBank_BANK:	//2
			if( CHAR_getWorkInt( talkerindex, CHAR_WORKSHOPRELEVANT) != 2 )	//錯誤程序
				return;
				...
				NPC_GambleBank_selectWindow( meindex, talkerindex, GAMBLE_END, flg);
				  ...
				  if( type < 7 && type >= 0 )	{
						  if( GB_ITEMS[page+type].name == NULL ||
							  ...
							  count = GB_ITEMS[page+type].Gnum;	//積分
...
int NPC_GambleBank_DoGold( int meindex, int toindex, int Gold, int flg)
{
	//flg = 1 正常 flg = 2 錢不夠 flg <= 0 錯誤 3 = 個人存款將滿 4 = 個人銀行將滿 5 = 單次最高金額
	char buf1[256];
...
//	int def = GAMBLEBANK_DEF;
	...
	if( Gold < 0 )	{	//取款
		...
		if( ( player_gold + Gold ) > CHAR_getMaxHaveGold(toindex) )	{
			return 3;	//取款後將超過個人金額
		}else	if( (Gold + stone_def)> CHAR_getInt( toindex, CHAR_PERSONAGOLD ) )	{	//銀行存款不夠
			return 2;
		}
		...
		sprintf( buf1,"取款：%d ，手續費：%d，銀行剩餘：%d 。", Gold, stone_def, CHAR_getInt( toindex, CHAR_PERSONAGOLD ));
		...
		sprintf( buf1,"存款：%d ，銀行剩餘：%d 。", Gold, CHAR_getInt( toindex, CHAR_PERSONAGOLD ));
		CHAR_talkToCli( toindex, meindex, buf1,  CHAR_COLORYELLOW);
		LogBankStone(    CHAR_getChar( toindex, CHAR_NAME ), CHAR_getChar( toindex, CHAR_CDKEY ),
					toindex, Gold, 
					"GB_Bank_save(銀行存款)", 
...
BOOL NPC_GambleBank_AddItem( int meindex, int toindex, int itemId, int count)
{
	...
	if( i == CHAR_MAXITEMHAVE )	{
		snprintf( token,sizeof( token), "物品欄空間不足！！");
		CHAR_talkToCli( toindex, -1, token,  CHAR_COLORWHITE);
		return FALSE;
	}
	...
	sprintf( token,"遊樂場積分剩餘： %d", CHAR_getInt( toindex, CHAR_GAMBLENUM));
	CHAR_talkToCli( toindex, -1,token,CHAR_COLORWHITE);
	return TRUE;
}
