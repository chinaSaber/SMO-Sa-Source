...
BOOL NPC_PairRegister( int toindex)
{
	...
	if( i>=MAXPAIRNUMS){
		if( space != -1 && CHAR_getWorkInt( toindex, CHAR_MYPAIRCODE) > 0 ){
			...
			CHAR_talkToCli( toindex, -1, "新增至配對名單。", CHAR_COLORYELLOW);
			return TRUE;
		}
	}
	return FALSE;
...
void NPC_PairUserAndWarp( int meindex, int toindex, int forindex, char *arg)
{
	...
	CHAR_DischargePartyNoMsg( toindex);//解散團隊
	CHAR_DischargePartyNoMsg( forindex);//解散團隊
...
int NPC_getPairCode( int toindex)
{
	...
	for( i=0; i<15; i++)	{
		...
		arg = ITEM_getChar(itemindex, ITEM_ARGUMENT );// 性別|個性編號|FLG
