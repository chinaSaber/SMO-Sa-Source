...
char UserBBIs[12][256]={
	"小矮子",	"賽亞人",	"辮子男孩",	"酷哥",			"熊皮男",	"大個",
	"小矮妹",	"熊皮妹",	"帽子妹",	"短法髮夾妹",	"手套女",	"辣妹" };
char PetBBIs[4][256]={//格魯西斯 貝魯卡 金格薩貝魯 貝魯伊卡
	"格魯西斯-紅虎",	"貝魯卡-綠虎",	"金格薩貝魯-金虎",	"貝魯伊卡-黃虎" };

int PlayerBBI[6*2][4]={
		{ 100000, 100005, 100010, 100015 },	//小矮子
		{ 100025, 100030, 100035, 100020 },	//賽亞人
		{ 100055, 100050, 100045, 100040 },	//辮子男孩
		{ 100060, 100065, 100070, 100075 },	//酷哥
		{ 100095, 100085, 100090, 100080 },	//熊皮男
		{ 100100, 100115, 100110, 100115 },	//大個

		{ 100135, 100120, 100125, 100130 },	//小矮妹
		{ 100145, 100140, 100150, 100155 },	//熊皮妹
		{ 100165, 100170, 100160, 100175 },	//帽子妹
		{ 100190, 100195, 100185, 100180 },	//短髮髮夾妹
...
BOOL CHECK_ReplacePET( int toindex); //更換寵物
void NPC_reCheckItemPilenum( int meindex, int toindex);//還原鐵槍三堆疊
BOOL CHECK_ITEMEQUIT( int toindex);//更換靈力鎧
BOOL CHECK_PETBBI( int toindex);//修正寵物圖號
...
//				  "\n            《還原鐵槍三堆疊》"
	...
	case WINDOW_SELECT2:
		...
		case 1://修正寵物異常
			...
			CHAR_talkToCli( toindex, -1, "身上寵物檢查完畢!", CHAR_COLORYELLOW);
			return;
			break;
		case 2://修正寵物圖號
		...
		case 3://更換靈力鎧
			if( CHECK_ITEMEQUIT( toindex) == TRUE ){
				CHAR_talkToCli( toindex, -1, "檢查完畢!", CHAR_COLORYELLOW);
			}
			return;
			break;
		...
		case 4:
			...
			if( NPC_reFindMyLostPet( meindex, toindex, token) == FALSE ){
				CHAR_talkToCli( toindex, -1, "查無資料！", CHAR_COLORYELLOW);
				return;
			}
			windowtype = WINDOW_MESSAGETYPE_SELECT;
			buttontype = WINDOW_BUTTONTYPE_CANCEL;
...
*/
	case WINDOW_SELECTLOSTPET:
#ifdef _PET_LOSTPET
		{
			...
			ti = flg;
			if( havepetelement < 0 ){
				CHAR_talkToCli( toindex, -1, "寵物欄位已滿！", CHAR_COLORYELLOW);
				return;
			}
			if( NPC_getLostPetString( meindex, toindex) == FALSE ) {
				CHAR_talkToCli( toindex, -1, "領取寵物失敗！", CHAR_COLORYELLOW);
				return;
			}
			...
			if( getStringFromIndexWithDelim( petstring[ti-1], "#", 2, petstring1, sizeof( petstring1)) == FALSE ){
				...
				if( CHAR_getInt( toindex, CHAR_GOLD) < cost ) {
					sprintf( buf1, "寵物已寄放%d天，共需%d石幣才可領回。", ltime, cost);
			...
			if( ret ) {
				...
				sprintf( buf1, "領取寵物%s。", CHAR_getUseName( petindex));
				CHAR_talkToCli( toindex, meindex, buf1, CHAR_COLORYELLOW);
...
void NPC_reCheckItemPilenum( int meindex, int toindex)
{
	...
	CHAR_talkToCli( toindex, -1, "鐵槍三處理完畢。", CHAR_COLORYELLOW);
}
