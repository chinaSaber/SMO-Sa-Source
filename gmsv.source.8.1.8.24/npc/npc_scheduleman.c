...
enum {
	...
	NPC_WORK_CHALLENGETIMEOUT = CHAR_NPCWORKINT2,	// 同意挑戰的 timeout
	NPC_WORK_SETTINGTIMEOUT = CHAR_NPCWORKINT3,	// 設定挑戰的 timeout
...
#ifdef _DEATH_FAMILY_CANT_SCHEDULE // WON ADD 家族戰玩家不能自行約戰
	  CHAR_talkToCli(talkerindex, meindex, "此星球無法約戰", CHAR_COLORWHITE);
	  break;
#endif
...
void NPC_RemoveExpiredBattle(int meindex)
{
  ...
  // 如果第一項排程與目前時間差異太大, 則整個排程全部無效
  if (expired==-1) {
    for (i=0; i<3; i++) {	  // tolerance 是 3 個 fight interval
      keeptime += fin;
