...
void NPC_SellsthMan_selectWindow( int meindex, int toindex, int num,int select, char *data)
{
	...
	case WINDOW_SELLON:
		if( CHAR_getWorkInt( toindex, CHAR_WORKSELLSTHNUM) >= 3 ){
			CHAR_talkToCli( toindex, meindex, "最多只能登記三次。", CHAR_COLORRED);
			return;
		}else if( CHAR_getInt( toindex, CHAR_GOLD) < 100 ){
			CHAR_talkToCli( toindex, meindex, "需付100石幣才能登記。", CHAR_COLORRED);
			return;
		}
	...
	case NPC_WINDOW_NEXT:
		{
			...
			}else if( select == WINDOW_BUTTONTYPE_OK ){ //買
			}else if( select == WINDOW_BUTTONTYPE_YES ){ //買
...
void NPC_SellsthManLoop( int meindex)
{
	...
	sprintf( headmsg, "拍賣頻道(%d頁%d項)：%s (拍賣者：%s)。",
	...
	if( NSellSthList[page][num].selltype==1 ){
		sprintf( token, "拍賣道具：%s。",
			ITEM_getChar( NSellSthList[page][num].Iindex, ITEM_NAME) );
	}else{
		sprintf( token, "拍賣寵物：%s  LV:%d %s。",
			CHAR_getChar( NSellSthList[page][num].Iindex, CHAR_NAME),
			CHAR_getInt( NSellSthList[page][num].Iindex, CHAR_LV),
#ifdef _PET_2TRANS
			CHAR_getInt( NSellSthList[page][num].Iindex , CHAR_TRANSMIGRATION) == 2 )?"二轉":(CHAR_getInt( NSellSthList[page][num].Iindex , CHAR_TRANSMIGRATION) == 1 )?"一轉":"")
#else
			( CHAR_getInt( NSellSthList[page][num].Iindex , CHAR_TRANSMIGRATION) == 1 )?"轉":""
#endif			
			);
	}
...
BOOL NPC_getSellSth_ViewString( int page, int num, char *ListString, int slen )
{
	...
	}else if( NSellSthList[page][num].selltype == 2 ){//寵物
		int petindex, i;
		ti = NSellSthList[page][num].uindex;
		petindex = CHAR_getCharPet( sellindex, ti);
		if( !CHAR_CHECKINDEX( petindex) ) return FALSE;
		//TYPE|名|  |圖|等級|轉|  |攻|防|敏|技1|技2|技3|技4|技5|技6|技7
