...
/*		CHAR_walk( index, dir, 0 );*/
	...
	for( i = 0; i < 3; i ++ ){
		...
		if( MAP_walkAble( index,fl,tX,tY ) == 0 ){
			aDirList[i] = -1; /* 墊仃卅中樺寧反公及  輊坫誚*/
//		}else if( NPC_Util_OtherCharCheck( fl,tX, tY ) ){
//			aDirList[i] = -2; /* 墊仃卅中樺寧反公及  輊坫誚*/
		}else{
			/* 墊仃凶!!*/
			OkFlg ++;
		}
...
/*-----------------------------------------------------------------------
* NPC及create凜及婁醒及  井日隙爛今木凶  儂  毛腹綢仄〕
* 公及  及醒襖毛潸  允月〔左皿撲亦件匹醒襖毛隙爛今六化支曰凶中午五卞忐厙
*
* 鏜啦「犯伉立正反"|"卞蜃爛仄化引允〔
*       婁醒卞  元  儂  互ㄡ勾岈氏分樺寧〕
*       燮及  互穸燮今木引允〔
...
/*
 * 失奶  丞及 index 午平乓仿 index 及孔凶勾及樹  井日｝
 * 平乓仿互公及失奶  丞毛  勻化月井升丹井仄日屯化  勻化凶日昇及
 * 匏  卞  勻化月井毛忒允〔  失奶  丞楮  及NPC迕〔CHAR_卞啖墊
 * 匹五月井手仄木氏楮醒瓜伉
 * by ringo
 *
...
* int	toindex		仇及平乓仿index卞覆仄化及  輊互菲戶日木月
* int	mode		潸  允月  輊
*                   0:toindex卞輊井勻化
*                   1:toindex及  覆  輊
*                   2:toindex卞覆仄化惘ㄨㄟ蘸  輊
...
*
* int	fromindex 	洶仁平乓仿index
* int	toindex		洶仁    及平乓仿index
* int	mode		洶仁  輊
*                   0:toindex卞輊井勻化
*                   1:toindex及  覆  輊
*                   2:toindex卞覆仄化惘ㄨㄟ蘸  輊
*                   3:toindex卞覆仄化爾ㄨㄟ蘸  輊
* BOOL	suberi		NPC_Util_SuberiWalk毛嗶  允月井〞TRUE:允月 FALSE:仄卅中
*
* 忒曰襖 岳    洶中凶  「dir  洶中凶  輊毛忒允  
*        撩            「-1
------------------------------------------------------------------------*/
